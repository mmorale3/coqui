...
#include<exception> // libc++ defines std::terminate here
#include<stdexcept>
#include<vector>
...
ContiguousIterator wait_any_n(ContiguousIterator it, Size n){
	...
	if(index == MPI_UNDEFINED) {throw std::runtime_error("cannot wait any: MPI_UNDEFINED");}
	if(index < 0) {throw std::runtime_error("cannot wait any: index < 0");}
	return it + index;
}
