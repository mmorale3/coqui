#!/usr/bin/env python3
"""Si HSE06 EOS in QE on the clean qe-7.5 build, for paw/uspp/oncv × 5 volumes.
Apples-to-apples vs VASP HSE06 (Schimka 2011: Si a0=5.435 A=10.27 Bohr) and vs our
...
for pp in ("paw", "uspp", "oncv"):
    for a in VOLS:
        d = os.path.join(DIAG, "hse_qe75", pp, f"a{a.replace('.','p')}")
        os.makedirs(d, exist_ok=True)
        open(os.path.join(d, "hse.in"), "w").write(hse(a, pp))
        tag = f"hse75_{pp}_a{a.replace('.','p')}"
        sb = "\n".join([
            "#!/bin/bash", f"#SBATCH -J {tag}", "#SBATCH -p ccq", "#SBATCH -N 1",
            "#SBATCH --exclusive", "#SBATCH -c 1", "#SBATCH -t 02:00:00",
            f"#SBATCH -o {d}/hse.%j.out", f"#SBATCH -e {d}/hse.%j.err",
            ...
            f"if [ ! -f hse.out.done ]; then mpirun -np 32 {PWX} -in hse.in > hse.out && touch hse.out.done; fi",
            'echo "end=$(date)"', ""])
        p = os.path.join(d, "run.sbatch")
        open(p, "w").write(sb); paths.append(p)
print(f"wrote {len(paths)} HSE06 run dirs under {DIAG}/hse_qe75")
