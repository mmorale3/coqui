#!/usr/bin/env bash
# Phase 0 schema validator for pw2coqui output.
#
# Usage: check_schema.sh <pwscf.coqui.h5>
#
# Exits 0 if the HDF5 file matches the Phase 0 schema (per
# notes/paw_implementation_plan.md), nonzero otherwise. Uses h5dump only —
# no Python / h5py dependency.
#
# What's checked:
...
#
# Not checked here (out of Phase 0): exact dataset shapes (size mismatches
# are caught later by the CoQui reader), multipole-moment reproduction
# (Phase 3 test), Onecenter symmetry (Phase 2 test).
...
echo "OK: $h5file matches Phase 0 schema (pp_type=$pp_type, $(wc -w <<<"$species_groups") species)"
