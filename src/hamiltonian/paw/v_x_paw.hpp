         ...
         bool onsite = true)
{
  ...
  // must NOT be added (it would double count).
  ...
  // QE's own paw_exx.f90 kernel (Eq. 32/33, Paier 2005) and VASP's PAW-EXX.
  // No explicit n_s ⇒ generalizes to
...
// time-reversal points — the same rule as compute_becsum_full_symm.
// ===========================================================================
inline void v_x(utils::mpi_context_t<boost::mpi3::communicator,
                                     boost::mpi3::shared_communicator>& mpi,
         pots::potential_t& vG,
