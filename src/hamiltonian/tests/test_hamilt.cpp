...
/**
 * Guards `thc_reader_t::select_aug_channels_qaware`, which replaced an
 * earlier q=0-only channel ranking.
 *
                              ...
                              double tol_VX_off_rel = 5e-3)
{
  ...
  // It showed up as 1.390937 only on a d-complete Si dataset (4x4x4,
...
TEST_CASE("thc_vs_direct_VH_VX", "[hamilt][thc][hf]")
{
  ...
  // The one-center exchange uses prefactor scl_oc=-1/N_k (v_x_paw.hpp).
...
// reproducing ABINIT's phiphj−tphitphj oscillator. Two checks on a PAW
// fixture:
...
//
// Motivation: an RPA instability on Si with a d-complete data set turned out
// to be a near-cancellation rather than a missing term. V_GL and V_LL are
// each ~2.3-2.4 Ha in the n=250->500 band increment
...
TEST_CASE("paw_thc_vs_exact_eri", "[hamilt][paw_erichk][!benchmark]")
{
  ...
  // ---- OFF-DIAGONAL ERI probe. Diagonal-only checks cannot see this element
  // class, and the term split below shows it is where the error lives:
  //
...
// Purpose: a reproducible baseline pair for the two operator conventions.
// NOTE: the quoted ABINIT reference values were produced on a different cell
// from the local qe_si222_paw fixture, so these numbers are regression
// baselines rather than a cross-code comparison.
