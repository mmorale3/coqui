...
same system; knobs are exposed as module constants):
...
# off-diagonal k-pair augmentation in exchange.
# --------------------------------------------------------------------------
def real_ylm(l, ghat):
...
def dense_sphere(ngfft, rprimd, recv):
    """G-sphere miller list: the largest sphere inscribed in the
...
def dense_sphere_ecut(ngfft, rprimd):
    ...
    reproduces the miller_g sphere."""
    gmax = min((ngfft[d] / 2.0) * 2.0 * np.pi / np.linalg.norm(rprimd[d])
               for d in range(3))
...
def write_hamiltonian_nc(h5root, w, vtrial, psps, rho_den=None, xc_name="pbe"):
    ...
    enables real vxc/vxc_with_nlcc export with functional xc_name.
    ...
    # nh(1:nsp) convention); projector_offset stays per-atom.
    ...
    # dense grid + local potentials (miller_g = ecutrho-style sphere)
    ...
    # inscribed-sphere cutoff) and drops the unused core-zeroed vxc dataset.
    ...
    # Hartree on disk (schema_version 2): readers skip the legacy
