 ...
 * hamilt_hf_sym_vs_nosym pins the general-nij symmetry lift (becsum + v_x)
 * against the nosym fixture.
 */
...
namespace bdft_tests {
  ...
  TEST_CASE("hamilt_hf_gygi_parity", "[methods][hamilt][hf][qe][paw]") {
    ...
    // operator level by the shared, route-free HF_K_correction, so the
    // direct-route gygi must match
