...
namespace bdft_tests {
  ...
  // ===========================================================================
  // Nonuniform-bosonic scGW on LiH222: same fixture as the periodic test
  // but with `make_nonuniform_log` activating the bosonic-Ω dense block.
  // Verifies the nonuniform-Ω path runs end-to-end without breaking the
  // kernels, and that the converged scGW state agrees with the uniform-Ω
  // run to within DIIS / quadrature tolerance.
  // ===========================================================================
  TEST_CASE("real_axis_scf_loop_lih222_nu_bosonic",
            "[real_axis][scf_loop][grid][thc][qe][bdft][periodic]")
  {
    ...
    // Nonuniform parameters: dense fermionic block + dense bosonic block.
    const double w_dense       = 0.5;
    const long   N_dense       = 33;   // matches parity of N_w (odd)
    const double Omega_dense   = Omega_max / 4.0;
    const long   N_Omega_dense = N_Omega / 2;     // 16 dense, 16 in tail
    ...
    // Sanity: confirm bosonic axis is actually nonuniform.
    {
      const auto& O = grid.Omega();
      const double dO_dense = O(1) - O(0);
      const double dO_tail  = O(N_Omega - 1) - O(N_Omega - 2);
      REQUIRE(dO_tail > 5.0 * dO_dense);
    }
    ...
    app_log(2, "[scf_loop_lih222_nu_bosonic] iter_used={}  ||dA||={:.3e}  mu={:.6f}",
            res.iter_used, res.final_diff, res.final_mu);

    // Sanity: SCF runs end-to-end and converges to the same regime as
    // the uniform-Ω test (which finishes at iter_used=19, ‖dA‖=7.9e-4,
    // μ=0.135). With the bosonic axis nonuniform the spectral structure
    // near low Ω is resolved differently, so the exact value of ‖dA‖ at
    // termination differs slightly — we only assert SCF stability.
    REQUIRE(res.iter_used >= 1);
    REQUIRE(res.iter_used <= cfg.max_iter);
    REQUIRE(std::isfinite(res.final_diff));
    REQUIRE(std::isfinite(res.final_mu));
    // mu should land in the gap. LiH222 KS gap midpoint is ~0.21 Ha; the
    // converged scGW μ should be within ~0.3 Ha of that.
    REQUIRE(std::abs(res.final_mu - mu0) < 0.3);
    REQUIRE(state.A_wskij.has_value());
    REQUIRE(state.ImSigma_wskij.has_value());
    REQUIRE(state.ReSigma_wskij.has_value());

    // Spectral function diagonals should be non-negative on average
    // (some negative values allowed due to finite-grid quadrature).
    auto A = state.A_wskij->local();
    long n_violations = 0;
    long n_total = 0;
    for (long iw = 0; iw < N_w; ++iw)
