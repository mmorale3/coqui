...
plan (device-direct chunked redistribution, device memory pool, device-resident
...
growing to 5--8$\times$ at production scale. The first and largest item is now
implemented and measured: a device-direct pairwise exchange cuts \texttt{update\_w} from
163 to 99 s/iter at 8 A100 and from 92 to 62 s/iter at 16, with energies unchanged digit
for digit. During the study we also found and fixed a
...
\section{Results of the first fixes (measured)}
\subsection{P1 and the leakage diagnostic}
Two changes account for a $2.2\times$ reduction of \texttt{update\_w} at 8 A100 ranks.
P1 (below) removed the host staging from the redistribute. Instrumenting what remained
then showed that the largest term in the $\tau\!\leftrightarrow\!\omega$ phases was not
communication at all but \texttt{IAFT::check\_leakage}, a \emph{diagnostic}: on device
input it began with \texttt{nda::to\_host} of the whole tensor (18.5 GB/rank) and then ran
two gemms over it on one host core, four times per iteration --- 28 s/iter. Its output is
three coefficient rows, $3/n_\tau$ of the tensor, so it now contracts on the device and
returns only those rows (\texttt{817df17}): 28.0 $\rightarrow$ 2.3 s/iter, diagnostic
value unchanged. Note the same \texttt{to\_host}-the-tensor pattern remains in
\texttt{simple\_dyson}, \texttt{scf\_common}, \texttt{thc\_sosex} and the embedding code.

\begin{table}[H]\small\centering
\begin{tabular}{@{}lrrr@{}}
\toprule
Per SCF iteration, 2 nodes $\cdot$ 8$\times$A100 & host staging & +P1 & +device leakage \\
\midrule
\textbf{update\_w} & 163.1 & 99.6 & \textcolor{good}{\textbf{74.3}} \\
\quad $\tau\rightarrow\omega$ & 52.0 & 21.0 & \textcolor{good}{10.8} \\
\quad $\omega\rightarrow\tau$ & 57.2 & 25.5 & \textcolor{good}{10.7} \\
\quad\quad of which redistribute (4 calls) & --- & 18.7 & 18.2 \\
\quad\quad of which \texttt{check\_leakage} & --- & 28.0 & \textcolor{good}{2.3} \\
\midrule
SCF loop, 2 iterations & 682.8 & 548.5 & \textcolor{good}{\textbf{493.5}} \\
\bottomrule
\end{tabular}
\caption{Cumulative effect of the two fixes. Energies are unchanged throughout
($0.48305038078408613$, then $0.5300765029178911$).}
\end{table}

\noindent Within the exchange itself, pack and unpack cost 0.02 s each per call and the
rest is time on the wire at 7.5--9.7 GB/s; a depth-2 software pipeline over the chunks
changed nothing (100.1 vs 99.9 s/iter), confirming the exchange is bandwidth-bound rather
than stalled. NCCL remains worth testing here, on the theory that it drives the same
hardware better than UCX does for this pattern.

\subsection{P1 in detail}
\begin{table}[H]\small\centering
\begin{tabular}{@{}lrrrrr@{}}
\toprule
& \multicolumn{2}{c}{2 nodes $\cdot$ 8$\times$A100} & \multicolumn{2}{c}{4 nodes $\cdot$ 16$\times$A100} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}
Seconds per SCF iteration & host staging & device-direct & host staging & device-direct \\
\midrule
\textbf{update\_w} & 163.1 & \textcolor{good}{\textbf{99.6}} & 92.2 & \textcolor{good}{\textbf{61.6}} \\
\quad $\tau\rightarrow\omega$ & 52.0 & \textcolor{good}{21.0} & 27.1 & \textcolor{good}{13.6} \\
\quad $\omega\rightarrow\tau$ & 57.2 & \textcolor{good}{25.5} & 32.2 & \textcolor{good}{17.5} \\
\quad W-Dyson solve & 17.6 & 17.4 & --- & --- \\
\quad eval $\Pi$ (RPA) & 22.0 & 21.4 & --- & --- \\
\midrule
SCF loop, 2 iterations & 682.8 & \textcolor{good}{\textbf{548.5}} & 429.2 & \textcolor{good}{\textbf{370.5}} \\
\bottomrule
\end{tabular}
\caption{\label{tab:p1}P1 measured: Si $2{\times}2{\times}2$/500 bands, shared ERI, two SCF
iterations, identical binary with only the transport switched. Energies agree digit for
digit ($0.48305038078408613$, then $0.5300765029178911$ at 8 ranks). The phases that
should not move do not. What remains in the transform phase is the IAFT gemms plus an
exchange that still serializes pack/send/unpack per chunk.}
\end{table}
...
\item[P1 --- Device-direct chunked redistribute. \statusfixed{9860888,\,c2f8e71} \textcolor{good}{measured $-$64 s/iter at 8 ranks, $-$31 s/iter at 16.}]
Implemented and now the default wherever the MPI advertises device-pointer support
(\texttt{MPIX\_Query\_cuda\_support}; \texttt{COQUI\_REDIST\_DEVICE} forces either path).
The DEVICE path walks peers in a fixed pairwise schedule --- round $r$ sends to
\texttt{rank}$+r$ and receives from \texttt{rank}$-r$ --- and hands device pointers
straight to CUDA-aware MPI. Only one peer block is in flight, so the extra device memory
is bounded by a chunk budget (\texttt{COQUI\_REDIST\_CHUNK\_MB}, default 1 GB per buffer)
rather than by the tensor size: exactly the bound the May attempt lacked. Over-budget
blocks split along their slowest dimension, and both sides stage through a C-ordered view
of the same global index box, so they derive the same split with no extra metadata
exchange. Measured (kp222/500b, shared ERI, A100): see
Table~\ref{tab:p1}. Bit-identical to host staging in every controlled comparison but
one, which differed by $3\times10^{-15}$ (round-off; total energies matched to all 16
digits). \textbf{NCCL was not needed} --- and is not available in these builds
(\texttt{NCCL\_LIBRARIES-NOTFOUND}); plain CUDA-aware MPI over UCX suffices, including at
16 ranks, where the ISDF builder's device-MPI crash (B3) notably does \emph{not} appear.
Remaining headroom: the exchange still serializes pack/send/unpack per chunk with a device
synchronize between them, so the phase sits at 47 s (8 ranks) rather than the 12 s a
bandwidth estimate allows; double-buffering across two streams is the next step.
\item[P2 --- Activate nda's existing pooled allocator. \textcolor{cpuc}{\textbf{built, opt-in, not usable at this scale}} (\texttt{ae0486f}, \texttt{975c912}, \texttt{9860888}).]
Built as described below and wired into the SCF loop as a scoped reservation
(\texttt{utils::device\_pool\_guard}, sized from \texttt{COQUI\_DEVICE\_POOL\_GB}), with
the ISDF builder's free-memory query made pool-aware. It serves the redistribute staging
buffers correctly (16 requests, zero fall-backs, $10^{-13}$-identical energies), but
reserving 3 GB inside the SCF loop at kp222/500b turned iteration 2 into a
\texttt{cudaErrorMemoryAllocation} while allocating an 18.5 GB tensor with 36.5 GB
reported free --- the failure mode the design notes below warn about. With 18.5 GB
tensors on an 80 GB card the headroom is simply not there, and P1's gain does not depend
on it, so the pool stays off by default until the peak footprint shrinks. The analysis
below stands and is worth keeping for that point.
...
\midrule
update\_w & 162 s & $\approx$38 s & redistributes 111$\to$\textbf{47 measured (P1)}$\to$12 with double-buffering, W-solve 17$\to$11, mirror/allocs 13$\to$1, $\Pi$ 20$\to$14 \\
...
\item \textbf{Week 1 --- \textcolor{good}{done}:} P2 pool built and wired (opt-in; not usable at
kp222/500b, see P2) $\rightarrow$ P1 device-direct redistribute implemented, validated
bit-identical, benchmarked at 8 and 16 ranks, and made the default where the MPI supports
it. Next in this line: double-buffer the exchange across two streams to close the
remaining 47\,s$\rightarrow$12\,s.
